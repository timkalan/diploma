...
in prejemati vso nagrado, jim pripada. Pri tem pa naletimo na problem. Agent lahko izkorišča te 
...
To je \textbf{Bellmanova enačba za Markovkse procese nagrajevanja}.
...
obstajajo iterativni algoritmi in metode, nekater izmed njih bomo spoznali v naslednjem odseku.
...
\begin{izrek}\label{optim}
    ...
    \begin{itemize}
        ...
        \item Vedno obstaja \textit{deterministična} optimalna stragegija. 
...
Še en pomemben razlikovalec pri reševanju MDP-jev je, ali je MDP \textit{poznan}, ali ne. Poznan 
MDP pomeni, da so nam znane vse komponente - dobro je definirana prehodna matrika $\mathcal{P}$.
Pogosto se pojavijo problemi, kjer te matrike ne poznamo, eden izmed takih problemov je tudi 
...
\begin{itemize}
    \item \textbf{Napovedovanje} - pri tem podproblemu podamo nek MDP in strategijo, algoritem nam 
            vrne vrednsotno funkcijo stanja (glede na strategijo).   
...
Dinamično programiranje je optimizacijska metoda deluje na principu deljenja velikega problema 
na manjše prekrivajoče podprobleme. V jedru reševanja je \textbf{Bellmanova enačba}, ki opisuje 
odnos med vrednostmi podproblemov in glavnega problema. Ker je idejo dinamičnega programiranja 
(DP) in MDP-jev dobil Bellman ob istem času, je naravno, da je DP metoda, ki je prilagojena prav 
situaciji v MDP-ju.

Problem mora v splošnem zadoščati dvem lastnostim, da je zanj primerno reševanje z dinamičnim 
programiranjem. Prvi pravimo \textbf{lastnost optimalne podstrukture}, ki pravi, da problem 
lahko razstavimo na podprobleme in rešitve podproblemov lahko potem sestavimo nazaj v rešitev
celotnega problema. Druga pomembna lastnost pa so \textbf{prekrivajoči podproblemi}, ki 
implicira, da lahko že poznane rešitve podproblemov večkrat uporabimo.
...
Podatki so tu poznan MDP in strategija $\pi$. Za algoritmom stoji Bellmanova enačba pričakovanja, 
z njo na vsakem koraku posodobimo $v_k(s) \rightarrow v_{k+1}(s)$ s pomočjo $v(s')$, kjer je $s'$
...
\begin{proof}
    ...
    naš rezultat (MOGOČE DEJMO NAVEST IZREK? - BANACH, CONTRACTION MAPPING).
    ...
    \begin{align*}
        ...
        &\leq ||\gamma \mathcal{P}_\pi ||v - u||_\infty||_\infty \\
...
S tem procesom tako res dobimo vrednsotno funkcijo, ki jo določa strategija, a strategija je 
...
Če se izboljšave nehajo, dobimo 
$$
...
bistvo pa črpajo pri dosedaj poznanem.
...
epizode dogajanja. Ideja delovanja je, da se namesto na direktno računanje pričakovanega povračilo, 
osredotočimo na računanje \textit{empiričnega} povračila: 
...
tako dobimo oceno za $v_\pi(s) \text{ } \forall s \in \mathcal{S}$.

Prostorsko zahtevnost lahko izboljšamo, če upoštevamo, da povprečje $\mu$ nekega zaporedja $X_1,
 X_2, \dots$ lahko izračunamo inkrementalno:
...
za vsako stanje $S_t$ s povračilom $G_t$ posodobimo: 
...
Posodobitveno pravilo \eqref{mc} je konkreten primer bistvene ideje spodbujevalnega učenja prek 
vrednostne funkcije. Vsi ostali algoritmi nam dajo pravilo podobne oblike: 
...
\begin{itemize}
    ...
    MC konvergira k rešitvi z minimalno povprečno kvadratno napako (MSE) $\sum_{k=1}^K 
    \sum_{t=1}^{T_k} (G_t^k - V(s_t^k))^2$ - najbolje se prilega opazovanim povračilom.

    TD(0) pa konvergira k rešitvi modela največjega verjetja Markova; to je rešitev MDP-ja, ki se 
    najbolje prilega podatkom. Dobimo torej naslednji cenilki: (TO SI ŠE POGLEJ)
...
V tej formulaciji pravila je tudi zelo očitno, zakaj ta algoritem poveže $TD(0)$ in $MC$:
