...
The motivation for this work was trying to undertand algorithms that learn through trial and error. 
In the beginning we lay the theoretical foundation by examining Markov decision processes. We then 
move on to derive and describe methods for solving them, which are based on dynamic programming. We
then generalize these methods and present three iterative algorithms: Monte Carlo, TD($0$) and
TD($\lambda$). Because we wanted to create a competent board game player, and board games often have 
a large number of states, we then look at function approximation and combining neural networks with 
...
tak način, da učenec maksimizira numerično nagrado. V kontekstu spodnujevalnega učenja učenca  
...
konca igre, tj. naj čim prej zmaga. Možna je tudi neničelna agrada za izenačenje, njen predznak pa je 
