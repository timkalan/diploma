...
in opisu metod za njihovo reševanje, ki temeljijo na konceptu dinamičnega programiranja. Te metode potem 
posplošimo in predstavimo tri glavne iterativne algoritme: Monte Carlo, TD($0$) in TD($\lambda$). 
Ker pa smo želeli ustvariti kompetentnega igralca namiznih iger, ki imajo pogosto veliko količino stanj, 
se nato posvetimo funkcijski aproksimaciji in kombinaciji nevronskih mrež s spoznanimi algoritmi.

V drugem delu naloge si bolj natančno ogledamo kombinatorne igre; to je teoretični model za namizne igre.
Nato opišemo nekaj pomembnih razlik, do katerih pride pri spodbujevalnem učenju v tem konteksu.

V zadnjem delu si ogledamo teorijo še na praktičnem primeru. Na $m$,$n$,$k$-igrah uporabimo opisane 
...
\vfill\noindent
{\bf Math. Subj. Class. (2010):} 62L20, 68T05, 90C40 \url{www.ams.org/mathscinet/msc/msc2010.html}  \\[1mm]  
...
%\begin{proof}
%    Ker funkcija $q_\pi(s, a)$ govori o pričakovanem povračilu, če sprejmemo akcijo $a$ in nato sledimo 
%    strategiji $\pi$, je optimalna funkcija definirana na enak način, in sicer kot $q_*(s, a) = E[R_{t+1} 
%    + \gamma v_*(S_{t+1})~|~S_t=s, A_t = a]$. Zaradi končnosti procesa, je to ekvivalentno
%    $$
%    q_*(s, a) = \mathcal{R}_s^a + \gamma \sum_{s' \in \mathcal{S}} \mathcal{P}_{ss'}^a v_*(s).
%    $$  
%    če začnemo z drugega zornega kota, 
%    \begin{enumerate}
%        \item Ker govorimo o končnih procesih s končnimi nagradami in ker velja da je $v_\pi(s) = \mathrm{E} 
%                [G_t~|~S_t = s]$ ter je $v_*$ maksimalna vrednostna funkcija, so vrednosti $v_*(s)$ navzgor 
%                omejene. Od tu takoj sledi obstoj vsaj ene strategije $\pi_*$.
%
%        \item Ker vrednostni funkciji govorita o pričakovanih povračilih, 
%                $\pi_*(a|s) = 
%                \begin{cases}
%                    1 & \text{če } a = \text{arg}\max_{a \in \mathcal{A}} q_*(s, a) \\
%                    0 & \text{sicer},
%                \end{cases}$
%
%        \item Ker velja, da $q_*(s, a)$ pove vrednost pričakovanega povračila, če izberemo akcijo $a$ in nato 
%                sledimo optimalni strategiji, potem velja enakost $q_*(s, a) = E[R_{t+1} + \gamma v_*(S_{t+1})~
%                |~S_t=s, A_t = a]$. Od tu sledi, da optimalna strategija določa enako $v_*$ in $q_*$.
%    \end{enumerate}
%    Ker velja $v_*(s) = \max_a q_*(s, a)$
%    DOKAZ SE KONČA TAKO DA SE IZPELJE BELLMANOVE ENAČBE, TO DA ENOLIČNOSTI IN DETERMINISTIČNOST
%\end{proof}
...
\begin{zgled}[Razlike med MC in TD -- ">ti si sodnik"< \cite{RLintro}]
    Recimo, da imamo na voljo za učenje naslednjih 8 epizod (vejice ločijo stanja in nagrade v epizodi, 
    podpičja pa dejanske epizode): 
    A, $0$, B, $0$;
    B, $1$;
    B, $1$;
    B, $1$;
...
samo ekstremna primera algoritma, ki ga bomo spoznali. V ta namen si poglejmo povračila, ki gledajo 
$n$-korakov v prihodnost. Tarča se primerno prilagodi v: 
$$
...
\end{equation}
To nas privede do ideje, da v do sedaj spoznanih algoritmih uporabimo funkcijo $q$ namesto $v$, 
...
rešitve zato potrebujejo veliko kolino podatkov, t.j. veliko količino epizod.  
...
Naletimo pa na nov problem; konvergenca namreč ni več nujno zagotovljena, vendar je vseeno lahko dosežena 
v posebnih primerih.
...
Kot posledica znanega rezultata iz teorije iger, izreka o minimaxu, je minimax vrednostna funkcija ena 
sama, minimax strategija, ki jo določa, pa je Nashevo ravnotežje. Tu je pomembno dejstvo, da govorimo 
o igrah z vsoto nič.
...
mnogo nedovoljenih stanj (npr. celotna plošča prekrita s križci), služi nam samo kot orientacija.
...
$3$,$3$,$3$-igro in $5$,$5$,$4$-igro. % in $6$,$7$,$4$-igro z dodatkom gravitacije.
...
nagrad je tudi enaka, kot je opisana zgoraj. To pomeni $1$ za zmago, $-1$ za poraz in $0$ sicer.
...
\begin{figure}[H]
    \includegraphics[scale=0.72]{../rezultati/tdl-333-40000-2.png}
...
\begin{figure}[H]
    \includegraphics[scale=0.72]{../rezultati/tdnn-333-40000-2.png}
...
\begin{figure}[H]
    \includegraphics[scale=0.72]{../rezultati/tdl-554-150000-2.png}
...
\begin{figure}[H]
    \includegraphics[scale=0.72]{../rezultati/tdnn-554-150000-2.png}
